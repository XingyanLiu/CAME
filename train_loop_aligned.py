...
#os.chdir('D:/lxy/scr4sent-20210420')
...
def foo_rmv(args):
    ...
    if key_class in adt.obs.columns:
        return utp.remove_adata_small_groups(
...
for rate in [1, 0.75, 0.5, 0.25, 0.1]:
    if rate < 1:
        sample_which = ['ref', 'que', 'refque'][0]
        tag = f'dcounts{rate:.2g}'
        resdir0 = RESDIR / 'dsample_counts' / f'{sample_which}-{tag}'
    else:
        sample_which = 'raw'
        resdir0 = RESDIR / 'dsample_counts' / f'{sample_which}'

    utp.check_dirs(resdir0)
    base.write_info(
            resdir0 / 'parameters.txt',
