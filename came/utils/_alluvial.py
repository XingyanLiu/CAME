...
class AlluvialTool:
    def __init__(
            self, input_data=(), x_range=(0, 1), res=20, 
            h_gap_frac=0.03, v_gap_frac=0.03, **kwargs):
...
#         }[type(self.input)]
    ...
    def get_color_array(self, colors=None, color_side=0, rand_seed=1, 
                        cmap=None, shuffle_colors=False,**kwargs):
        _ = kwargs
        ...
        if colors is None:
#            print('TEST: `get_color_array`')
            cids = np.linspace(0, 1, lci)
            if shuffle_colors: np.random.shuffle(cids)
            color_array = [cmap(item) for ind, item in enumerate(cids)]
        else:
            color_array = colors
#        color_array = colors if colors is not None else [
#            cmap(item) for ind, item in enumerate(cids)]
    ...
    def label_sides(
            self, labels=None, label_shift=0, disp_width=False, 
            wdisp_sep=7*' ', fontname='Arial', **kwargs):
        if labels is not None:
            ...
            for side, sign in enumerate((-1, 1)):
                plt.text(
                    ...
                    fontsize=13, rotation=90-180*side
                )
