...
=============================================
1. max pred-probas for each test samples
2. compared with the specific threshold corresponding to that predicted class
3. reject
4. return the index of unknown samples
'''
def predict_probas(logits, softmax=True, only_max=True):
    if softmax:
        _probas_all = F.softmax(logits.cpu().clone().detach(), dim = 1)
    else:
        _probas_all = logits.cpu().clone().detach()
...
#    print(n_classes)
    ...
    for cl in range(n_classes):
        ...
        if ind_class_right.sum() == 0:
            print(f'no samples in the training data are rightly labeled as {cl}, skipped')
            continue
        ### NOTE: 
    ...
    if thd_max is not None:
        ...
        print(f'thresholds are clipped, in a range of [{thd_min:.4g}, {thd_max:.4g}]')
    return thresholds
...
def predict_with_unknown(logits: torch.Tensor, 
    ...
    _train_idx, _test_idx, _train_labels0 = tuple(
                map(lambda x: x.cpu().clone().detach() if isinstance(x, torch.Tensor) else x, 
