...
def merge_adatas(adatas: Union[Mapping[str, sc.AnnData], 
                               Sequence[sc.AnnData]], 
                 union: bool = True, 
                 obs_keys=None, 
                 dsnames=None,  
    ...
    union: whether to take union of the features (e.g. genes)
...
def merge_adata_groups(adata: sc.AnnData, key: str, 
                       group_lists: Sequence, 
                       new_key = None, 
...
def group_zscore_adata(adt, key='counts', groupby='batch', key_new = None, 
    ...
    if key == 'counts':
        ...
        if sparse.issparse(adt.X): X = adt.X.toarray()
        else: X = adt.X
